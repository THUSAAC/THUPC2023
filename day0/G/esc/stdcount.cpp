			...
			if(ly[l][r]>r){
				...
				for(int k=l;k<=r;++k){
					...
					if(op[k]>rmax){
						//++visit;
						if(rx<k){
							f[l][r][0]+=f[l][k-1][1]*f[k+1][op[k]-1][1]*f[op[k]+1][r][BOTH];
						}
						if((xl[k]==l||r<rr[k])&&rx<op[k]){
							f[l][r][0]+=f[l][k-1][0]*f[k+1][op[k]-1][1]*f[op[k]+1][r][BOTH];
						}
						rmax=op[k];
					}
				}
				if(xl[r]!=l){
					...
					if(xl[xlp-1]==l||r<rr[xlp-1]){
						f[l][r][0]+=f[l][xlp-1][0]*f[xlp+1][r][BOTH];
					}
				}
				f[l][r][1]=f[l][r][0];
			}
