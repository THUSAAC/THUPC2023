...
void get_average_skill_effect(int team){//回合结束时，触发average的主动技能的掉血效果
	for(int i = 1;i <= n[team];++i){
		...
		//x.out();
		output.output_dmg_skill(x,x.average_skill_effect / 100.0l * x.maxhp + eps);
...
void attack(Player &plr,Player &tar,db basic_val,int atkpos,int ddgpos){//攻击结算，basic_val是基础攻击强度
	...
	output.output_dmg(plr,tar,true_val / (tar.def * tar.team_def) + eps);
...
void skill(Player &plr,Player &tar){//发动主动技能
	...
	if(plr.type == WEAK){
		output.output_recovery(tar,weak_skill_val[plr.skill_lv] / 100.0l * tar.maxhp + eps);//weak主动技能，让队友回血
	}
