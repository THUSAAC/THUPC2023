...
int main(){
	init();
    int basiclvl=R(95,98);
	int n=R(5,6),m=n;
    printf("%d %d\n",n,m);
	team[0].cnt=n;
	team[1].cnt=m;
	for(int i=0;i<n;++i){
		Player& player=team[0].player[notopos[i]];
		...
		player.passivelevel=passivelevel;
        ...
        double weaponatk=std::uniform_real_distribution<>(0.04*health,0.06*health)(gen);
        cmax(weaponatk,300);
        cmin(weaponatk,80000);
        Real realwpatk(weaponatk);
		player.realwpatk=realwpatk;
		if(*token=='B'){
	...
	for(int i=0;i<m;++i){
		Player& player=team[1].player[notopos[i]];
		player=team[0].player[notopos[i]];
		player.team=team+1;
		int type=player.type+1;
		if(type==1){
            strcpy(token,"Weak");
        }
        if(type==2){
		...
		int weapontype;
		if(dynamic_cast<Bow*>(player.weapon))weapontype=0;
			else if(dynamic_cast<Gun*>(player.weapon))weapontype=1;
			else weapontype=2;
		printf("%s Lv=%d maxhp=%d atk=%s def=%s skillLv=%d passivesklLv=%d %c weaponatk=%s\n",token,player.lv,player.maxhp,std::string(player.atk).c_str(),std::string(player.def).c_str(),player.skill->level,player.passivelevel,"BGM"[weapontype],std::string(player.realwpatk).c_str());
	}
